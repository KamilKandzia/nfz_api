...
monthlyProvision_server <- function(input, output, session, Suggestions, allSuggestion,Id049, dose, daterange1, dictType) {
  
  ns = session$ns
  
  server_legend<- reactive({input$legends_month}) 
  server_label<- reactive({input$labels_month}) 
  
  
  observeEvent(input$analyseMonthly,{
    ...
    withProgress(message = 'Making API GET content', value = 0, {
      ...
      foreach(Product=Suggestions()) %do%{
        
        for(NFZdiv in c(1:16)){
          incProgress(1/(length(Suggestions())*16), detail = "NFZ division")
          
          NFZdivision<-concatString(NFZdiv)
          
          req_parsed<-searchNFZ(Id049(), NFZdivision, "province", Product, daterange1(), dose(), dictType, "monthly-provisions", total=NULL, NFZdiv)
          req_parsed$data[,"NFZdivision"]<-NFZdiv
          req_parsed$data[,"Product"]<-Product
    ...
    output$tbl_monthly <- renderDT(
      keks, options = list(lengthChange = FALSE, scrollX = TRUE)
    )
    
    
    #####BETTEER CHART
    XD<-agreggateTableMonthlyProvisions(keks, "quantity")
    
    renderBarChart(div_id = "test1_monthly", grid_left = '1%', direction = "vertical",
                   data = XD,axis.x.name = "month",axis.y.name = "quantity",show.legend = as.logical(server_legend()))
    ...
    output$tbl_monthly_values <- renderDT(
      XDE, options = list(lengthChange = FALSE, scrollX = TRUE))
    
    output$vbox_month <- renderbs4ValueBox({
      bs4ValueBox(
    ...
    ########################### REFUND
    XD<-agreggateTableMonthlyProvisions(keks, "refund")
    
    renderBarChart(div_id = "test_monthly", grid_left = '1%', direction = "vertical",
                   data = XD,axis.x.name = "month",axis.y.name = "refund",show.legend = as.logical(server_legend()))
    ...
    output$plt_monthly2017 <- renderPlot({
    ...
    output$plt_monthly2018 <- renderPlot({
